...
open class EmbeddingService(
    ...
    open fun storeDatasetEmbedding(
        ...
        val summary =
            ...
            Datasettet har ${dataset.accessRights
                ?.prefLabel
                ?.valueByPriority()
                ?.lowercase() ?: "ukjent"} tilgang.
            ...
            Datasettet har ${dataset.distribution?.size ?: 0} distribusjoner${ if (formats.isNotEmpty()) {
                " og tilbyr data på formatene ${formats.joinToString(
                    ", ",
                )}"
            } else {
                ""
    ...
    open fun storeDataServiceEmbedding(
        ...
        val summary =
            ...
            Datatjenesten har ${dataService.accessRights
                ?.prefLabel
                ?.valueByPriority()
                ?.lowercase() ?: "ukjent"} tilgang.
    ...
    open fun storeInformationModelEmbedding(
        ...
        val summary =
            ...
            Informasjonsmodellen har ${informationModel.accessRights
                ?.prefLabel
                ?.valueByPriority()
                ?.lowercase() ?: "ukjent"} tilgang.
            ...
            ${ if (!informationModel.subjects.isNullOrEmpty()) {
                "Informasjonsmodellen omhandler emnene: ${informationModel.subjects.joinToString(
                    ", ",
                )}."
            } else {
                ""
    ...
    open fun storeServiceEmbedding(
        ...
        val authorities =
            service.hasCompetentAuthority?.mapNotNull { it.prefLabel?.valueByPriority() ?: it.title?.valueByPriority() } ?: emptyList()
    ...
    fun markDeletedByIdAndBeforeTimestamp(
        ...
        if (!deleted) {
            logger.debug(
                "Skipped marking embedding as deleted for {} - message timestamp {} is not newer than existing embedding",
                id,
                timestamp,
            )
        }
